...
class Products
{
	...
	public function getUnits(){
		$q = $this->con->query("SELECT * FROM units");
		$ar = [];
		if ($q->num_rows > 0) {
			while ($row = $q->fetch_assoc()) {
				$ar[] = $row;
	...
	public function addUnit($name){
		$q = $this->con->query("SELECT * FROM units WHERE unit_title = '$name' LIMIT 1");
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Unit already exists'];
		}else{
			$q = $this->con->query("INSERT INTO units (unit_title) VALUES ('$name')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'New Unit added Successfully'];
			}else{
				return ['status'=> 303, 'message'=> 'Failed to run query'];
			}
		}
	...
	public function deleteUnit($bid = null){
		if ($bid != null) {
			$q = $this->con->query("DELETE FROM units WHERE unit_id = '$bid'");
			if ($q) {
				return ['status'=> 202, 'message'=> 'Unit removed'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to run query'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'Invalid unit id'];
		}
	...
	public function updateUnit($post = null){
		extract($post);
		if (!empty($unit_id) && !empty($e_unit_title)) {
			$q = $this->con->query("UPDATE units SET unit_title = '$e_unit_title' WHERE unit_id = '$unit_id'");
			if ($q) {
				return ['status'=> 202, 'message'=> 'Unit updated'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to run query'];
			}
...
if (isset($_POST['add_unit'])) {
	if (isset($_SESSION['admin_id'])) {
		...
		if (!empty($unit_title)) {
			$p = new Products();
			echo json_encode($p->addUnit($unit_title));
		}else{
			echo json_encode(['status'=> 303, 'message'=> 'Empty fields']);
		}
	}else{
...
if (isset($_POST['DELETE_UNIT'])) {
	if (!empty($_POST['bid'])) {
		$p = new Products();
		echo json_encode($p->deleteUnit($_POST['bid']));
		exit();
	}else{
		echo json_encode(['status'=> 303, 'message'=> 'Invalid details']);
		exit();
...
if (isset($_POST['edit_unit'])) {
	if (!empty($_POST['unit_id'])) {
		$p = new Products();
		echo json_encode($p->updateUnit($_POST));
		exit();
	}else{
		echo json_encode(['status'=> 303, 'message'=> 'Invalid details']);
		exit();
